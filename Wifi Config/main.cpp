...
static void buildBody() {
  ...
  lv_label_set_text(label, "Please click the setting button to setup your wifi!");
  lv_obj_center(label);*/

  wfList = lv_list_create(bodyScreen);
  //lv_obj_set_size(wfList, tft.width() - 140, 210);
  lv_obj_set_size(wfList, tft.width() - 33, tft.height() - 52);
...
static void buildSettings() {
  ...
  //lv_obj_set_size(wfList, tft.width() - 140, 210);
  lv_obj_set_size(wfList, tft.width() - 140, 160);
